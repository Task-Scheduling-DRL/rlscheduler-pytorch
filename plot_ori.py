    ...
    for root, _, files in os.walk(logdir):
        if "progress.txt" in files:
            ...
            if other_algos:
                ...
                exp_data3.insert(
                    len(exp_data3.columns), "Performance", -exp_data["SJF"]
                )
...
def get_all_datasets(
    all_logdirs, legend=None, select=None, exclude=None, other_algos=False
):
    """
...
def make_plots(
    ...
    data = get_all_datasets(
        all_logdirs, legend, select, exclude, other_algos=other_algos
    )
