...
def mpi_fork(n, bind_to_core=False):
    ...
    if os.getenv("IN_MPI") is None:
        env = os.environ.copy() # os.environ: 운영체제가 참조하는 변수
        env.update(
            MKL_NUM_THREADS="1",
            OMP_NUM_THREADS="1",
            IN_MPI="1"
...
def mpi_op(x, op):
    ...
    x = np.asarray(x, dtype=np.float32) # asarray는 array와 다르게 원본 변경시 복사본도 변경된다.
...
class Logger:
    ...
    def __init__(self,
                 output_dir=None,
                 output_fname='progress.txt', # 로깅 출력 이름 변경하는 곳
                 exp_name=None):
        """
        Initialize a Logger.
...
def count_vars(module):
    return sum([np.prod(p.shape) for p in module.parameters()]) # np.prod: 객체 행 또는 열(axis=1)의 요소들을 곱한 값 반환
