...
class Job:
    ...
    def __str__(self):
        return (
            ...
            + str(self.job_id)
            + "]-["
            + str(self.request_number_of_processors)
            + "]-["
...
class Cluster:
    ...
    def can_allocated(self, job):
        if (
            job.request_number_of_nodes != -1
            and job.request_number_of_nodes > self.free_node
        ):
            return False
        if (
            job.request_number_of_nodes != -1
    ...
    def allocate(self, job_id, request_num_procs):
        ...
        print(
            "Error in allocation, there are enough free resources but can not allocated!"
        )
        return []
...
class SchedGym(gym.Env):
    def __init__(
        ...
        super(SchedGym, self).__init__()
        print("Initialize Simple HPC Env")

        self.action_space = spaces.Discrete(MAX_QUEUE_SIZE)  # discrete 일때
    ...
    def my_init(self, workload_file="", sched_file=""):
        ...
        if self.build_sjf:  # this is for trajectory filtering.
            ...
            if self.batch_job_slice == 0:
                ...
                max_index = (
                    min(self.batch_job_slice, self.loads.size()) - JOB_SEQUENCE_SIZE - 1
                )
    ...
    def reset(self):
        ...
        # scheduled_scores 변수로 점수 추가하기 (f1, f2, f3, f4) 각각 어떤 점수에 해당하는지 알아볼 필요 있음
...
def test_hpc_env():
    ...
    env = SchedGym(batch_job_slice=100, build_sjf=True)
    env.seed(0)
