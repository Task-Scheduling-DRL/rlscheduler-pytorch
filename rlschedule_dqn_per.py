...
class MemoryBuffer:
    ...
    def __init__(
        ...
        )  # try with   dtype=np.bool
        self.new_obs = np.zeros((buffer_size, obs_dim), dtype=np.float32)

        # SumTree 추가
        # self.tree_q = SumTree(buffer_size)
        self.tree_td = SumTree(buffer_size)

    # ver 2
    # def _get_priority_q(self, q_values):
    #     return 1 / (1 + np.exp(-np.abs(q_values)))

    def _get_priority_td(self, error):
        return (1 / (1 + np.exp(-np.abs(error)))) * 2 - 1

    # PER 위해 추가
    # def _get_priority_q(self, q_values):
    #     return (np.abs(q_values) + self.e) ** self.a
    ...
    # PER에 맞게 수정한 방식
    def store_memory(
        self, error, obs, action, reward, done, new_obs
    ) -> None:  # q_value 추가해야함
        sample = (obs, action, reward, done, new_obs)
        p_td = self._get_priority_td(error)
        # p_q = self._get_priority_q(q_values)
        # self.tree_q.add(p_q, sample)
        self.tree_td.add(p_td, sample)
    ...
    # PER에 맞게 수정한 방식
    def get_samples(self, epsilon_o):
        batch = []
        idxs = []
        # segment_q = self.tree_q.total() / self.batch_size
        segment_td = self.tree_td.total() / self.batch_size
        priorities_q = []
        priorities_td = []
        ...
        """
            ...
            To sample a minibatch of size k, the range [0, p_total] is divided equally into k ranges. (1)
            (p_total = self.tree_q.total())
            Next, a value is iniformly sampled from each range. (2)

            -> [0, p_total]을 k개의 동일한 크기의 구간으로 나누고 (1) 각 segment에서 하나씩 uniform하게 샘플링 (2)
        """
        for i in range(self.batch_size):
            ...
            """while True:
                # (2)
                s_q = random.uniform(a_q, b_q)
                (idx, p_q, data) = self.tree_q.get(s_q)
                if not isinstance(data, int):
                    break
            while True:
                # (2)
                s_td = random.uniform(a_td, b_td)
                (idx, p_td, data) = self.tree_td.get(s_td)
                if not isinstance(data, int):
                    break"""
            # while True:
            #     s_q = random.uniform(a_q, b_q)
            ...
            if self.count % 500000 == 0:
                # logger.info("tree[ : 50] : %s", self.tree_td.tree[:20])
                # logger.info("tree[-50 : ] : %s", self.tree_td.tree[-20:])
                # logger.info("count %s", self.count)
                # logger.info("p : %s", p_td)
            # priorities_q.append(p_q)
            priorities_td.append(p_td)
            # 08/20 18:07 임시 추가
            # int type data 확인됨
            batch.append(data)
            idxs.append(idx)
        # sampling_probabilities = float(epsilon_o) * (
        #     priorities_q / self.tree_q.total()
        # ) + (1.0 - float(epsilon_o)) * (priorities_td / self.tree_td.total())
        sampling_probabilities = float(epsilon_o) * (priorities_td / self.tree_td.total())
        epsilon = 1e-10
        sampling_probabilities = np.where(
            sampling_probabilities == 0, epsilon, sampling_probabilities
        )
        ...
        is_weight = np.power(
            self.tree_td.n_entries * sampling_probabilities, -self.beta
        )
    ...
    # PER 적용 위해 추가
    def update(self, idx, error): # 마지막 입력값에 추가 q_values
        # p_q = self._get_priority_q(q_values)
        p_td = self._get_priority_td(error)
        # logger.info("%s", p_q)
        # self.tree_q.update(idx, p_q)
        self.tree_td.update(idx, p_td)
...
class DQN:
    ...
    def __init__(self, ac_kwargs=dict(), logger_kwargs=dict()):
        """
        ...
        self.remaining_progress = (
            1  # tracks how much % of total steps remain -> value between 1 and 0
        )
        self.num_timesteps = 0
        self.n_updates = 0

        self.logger = EpochLogger(**logger_kwargs) # 확인
    ...
    def predict(
        ...
        action_mask: np.ndarray = np.ones(1),
        ...
        """
        observation = T.tensor(np.array([observation]), dtype=T.float).to(
            self.q_net.device
        )
    ...
    def train(self) -> None:
        ...
        for _ in range(self.gradient_steps):
            ...
            # 수정
            batch, idxs, is_weights = self.memory_buffer.get_samples(self.epsilon)
            ...
            for idx, sample in enumerate(batch):
                ...
                obs, action, reward, done, new_obs = batch[idx]
            ...
            new_obs = T.tensor(new_obs_array, dtype=T.float).to(
                self.q_target_net.device
            )
            ...
            # 추가
            q_value_eval = T.abs(current_q_values).data.numpy()
            errors = T.abs(current_q_values - target_q_values).data.numpy()

            # priority 업데이트
            for i in range(self.batch_size):
                idx = idxs[i]
                self.memory_buffer.update(idx, errors[i], q_value_eval[i])
            ...
            # 수정
            loss = (
                T.FloatTensor(is_weights)
                * F.mse_loss(current_q_values, target_q_values)
            ).mean()
    ...
    def append_sample(self, state, action, reward, done, next_state):
        ...
        state = T.tensor(state, dtype=T.float).to(self.q_target_net.device)
        ...
        next_state = T.tensor(next_state, dtype=T.float).to(self.q_target_net.device)
        ...
        q_values_eval = T.tensor([current_q_values], dtype=T.float).to(
            self.q_target_net.device
        )
        error = abs(current_q_values[0].item() - target_q_values[0].item())

        self.memory_buffer.store_memory(
            q_values_eval, error, state, action, reward, done, next_state
        )

    def learn(
        self, total_instances: int, total_timesteps: int, intermediate_test=None
    ) -> None:
        """
        ...
        for _ in range(total_instances):
            ...
            while not done:
                ...
                if (
                    self.num_timesteps >= self.training_starts
